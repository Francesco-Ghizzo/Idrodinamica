...
\end{equation}
\noindent dove $i$ rappresenta l'indice non ripetuto e $k$ rappresenta l'indice ripetuto.\\
Il problema più comune associato a tali sistemi è il seguente problema di valore iniziale (PVI): dati i valori della funzione sconosciuta al tempo $t=0$ $U(x,0)$, usare le equazioni per determinare l’evoluzione nel tempo della funzione $U(x,t)$ (Ben-Artzi \& Falcovitz, 2003).\\
...
\begin{equation}
    \frac{D}{Dt}=\frac{\partial }{\partial t}+\left(v\cdot\nabla\right)
    \label{eqn:derivata_materiale_3D}
\end{equation}
\noindent dove:
\begin{itemize}
    \item $v$ è il vettore velocità del sistema di riferimento lagrangiano [$m\cdot s^{-1}$]
\end{itemize}
...
\begin{minipage}[b]{8.5cm}
    \centering
    \includegraphics[height=6cm, width=8.5cm]{Punticappio.png}
