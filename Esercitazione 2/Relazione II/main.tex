...
\noindent Il presente studio si prefigge come obiettivo condurre simulazioni di profili di correnti in differenti condizioni sperimentali.
...
\begin{equation}
    U=ks\sqrt{i_f}R_i^{2/3}
    \label{eqn:U_Gauckler-Strickler}
\end{equation}

\subsubsection{Stima della scabrezza in alveo rettangolare}

\noindent Supponiamo di trovarci in condizioni di alveo rettangolare e moto uniforme a portata assegnata. Sia inoltre sconosciuto il valore della scabrezza del fondo. Ci proponiamo di stimare il valore del coefficiente di Gauckler-Strickler del fondo a partire dai valori misurati di tirante. 
...
Questa equazione è valida in condizioni di moto gradualmente variato, ovvero quando le variazioni di sezione e direzione sono graduali, tali per cui le linee di corrente risultano essere essenzialmente rettilinee e parallele tra loro.
...
%    \centering
%    \begin{tabular}{|c|}
%        \includegraphics[scale=1.1]{TabellaFTF.png}
%    \end{tabular}
%    \caption{Dati misurati e profondità calcolata - caso FTF}
%    \label{tab:FTF}
%\end{table}
...
\noindent valida per qualsiasi alveo rettangolare. Una volta tracciata la profondità critica (Figura \ref{fig:critica_FTF}), dal raffronto con le quote di tirante misurate notiamo che vi sono due passaggi della corrente per lo stato critico in prossimità nei cambi di pendenza, il che evidenzia la condizione torrentizia dell'alveo centrale.
...
Nel nostro caso il profilo calcolato risulta molto incerto, anche se il fenomeno dissipativo sembra avere inizio appena a monte del cambio di pendenza. Questo comporterebbe la formazione di un profilo di tipo $S1$. A conferma di ciò è possibile effettuare una verifica analitica tramite l'utilizzo della formula della spinta: 
...
\noindent con $\rho$ densità dell'acqua, g costante di gravità, B larghezza dell'alveo, Y tirante di moto uniforme e Q portata.

\subsection{Stima della scabrezza}

\noindent Il valore del coeefficiente di Gauckler-Strickler $ks$ è stato stimato attraverso due approcci, che sono stati infine confrontati. Supponendo che le pareti della canaletta siano sufficientemente lisce, infatti, è possibile trattare la canaletta come un alveo rettangolare largo, ovvero come un alveo in cui le tensioni ai lati siano trascurabili ed il cui perimetro bagnato sia approssimabile alla larghezza del fondo. In questo caso, assumendo che nel tratto centrale della canaletta si sviluppi il moto uniforme, è possibile calcolare il tirante idraulico per via analitica mediante la formula:

\begin{equation}
    Y=\left(\frac{Q}{KsB\sqrt{i_f}}\right)^\frac{3}{5}
\end{equation}

\noindent Dal momento che le altre variabili sono note, il valore di $ks$ è stimabile semplicemente inserendo nell'equazione dei valori di scabrezza variabili e confrontando il tirante calcolato con il pelo libero misurato.\\
Un approccio più rigoroso è quello di modellare la canaletta attraverso un algoritmo o software dedicato e stimare la scabrezza della seconda livelletta a partire dal valore della spinta, i cui effetti sono misurabili nello spostamento a valle del risalto: come detto in precedenza, maggiore sarà la spinta, più a valle sarà localizzato il risalto. Il valore del coefficiente di Strickler $ks$ potrà pertanto essere stimato simulando per la canaletta diversi valori di $ks$, fintantoché non vi sia una buona corrispondenza tra la localizzazione del risalto nei dati reali e simulati.\\
...
I dati ricavati dalle varie esperienze sono stati plottati tramite Excel per ottenere un riscontro grafico apprezzabile della variazione di posizione del risalto al variare della scabrezza del fondo centrale:
...
\noindent Nella prima e nella seconda livelletta, pertanto, vedremo svilupparsi un moto uniforme, anche se con profondità minore nella seconda livelletta per via della maggiore pendenza del tratto centrale. Nella terza livelletta la corrente si trova costretta a passare \textit{ex abrupto} da una condizione uniforme supercritica ad una quota subcritica determinata dalla parete di fondo dell'apparato, generando così un risalto idraulico, in questo caso chiaramente posizionato a valle del cambio di pendenza, seguito da un profilo veloce decelerato $S1$ di raccordo col tirante finale. \\
...
%    \centering
%    \begin{tabular}{|c|}
%        \includegraphics[scale=1.1]{TabellaTTT.png}
%    \end{tabular}
...
\subsection{Stima della scabrezza}
\noindent Il valore di ks è stato stimato modellando la canaletta grazie al software HEC-RAS e comparando la localizzazione dell’inizio del risalto idraulico nel profilo di corrente simulato e misurato in laboratorio. Il modello su acqua bassa è inutilizzabile dal momento che, nella terza livelletta, il profilo della corrente non è approssimabile con una corrente di moto uniforme. Le condizioni di contorno utilizzate sono state inizialmente il tirante misurato sia a valle che a monte. Tuttavia, a partire da queste condizioni di contorno, non vi è stata corrispondenza nella localizzazione del risalto, pur essendo stati effettuati molti tentativi anche con valori di ks molto alti, in un intervallo che va da 80 fino a 300 (a scopo puramente dimostrativo). Anche in questo caso è stato eseguito un confronto grafico tramite Excel:
...
Con le ipotesi di modello di acqua bassa, invece, le iterazioni sono dovute continuare sino a raggiungere valori di ks pari circa a 200, scabrezza non raggiungibile dai materiali utilizzati. Da questo è possibile intuire come il modello su acqua bassa non sia una buona approssimazione per la canaletta in esame. 
...
\noindent Nella seguente tabella si riportano i valori di tirante uniforme misurato e di scabrezza ottenuti per la seconda livelletta, oltre ai valori di spinta delle correnti della seconda e terza livelletta:

\begin{table}[H]
    \centering
    \begin{tabular}{|c|c|}
        \hline
        \textbf{Quota tirante [m]} & $0,032$ \\
        \textbf{Ks [$\frac{m^\frac{1}{3}}{s}$]} & $100$\\
        \textbf{Spinta monte [kN]} & $237$ \\
        \textbf{Spinta valle [kN]} & $942$\\
        \hline
    \end{tabular}
...
\noindent  Dai risultati ottenuti non è stato possibile ottenere una perfetta corrispondenza con l’inizio del risalto misurato, nemmeno con valori di ks molto elevati, perciò è stato assunto un valore di scabrezza realistico pari a 100. 
...
\noindent Come nel caso precedente, nella seguente tabella si riportano i valori di tirante uniforme misurato e di scabrezza ottenuti per la seconda livelletta, tranne i valori di spinta monte/valle per cui non è possibile operare un confronto:

\begin{table}[H]
    \centering
    \begin{tabular}{|c|c|}
        \hline
        \textbf{Quota tirante [m]} & $0,04$ \\
        \textbf{Ks [$\frac{m^\frac{1}{3}}{s}$]} & $100$\\
        \hline
    \end{tabular}
    \caption{Portata critica in funzione dei tempi di ritorno}
...
\subsection{Inquadramento territoriale}
...
\noindent La prima fase prevede la modellazione delle geometrie del tratto torrentizio da analizzare. Le sezioni di interesse, in totale 35 ricavate tramite rilevamento sul campo, vengono fornite con file .dwg e importate su AutoCad per studiarne le caratteristiche geometriche attraverso la planimetria e valutare la scabrezza delle singole componenti. 

\begin{figure}[H]
    \centering
    \includegraphics[scale=0.5]{Sezione.jpg}
...
\noindent La determinazione della scabrosità dell'alveo più adatta alla condizione reale avviene suddividendo il terreno nei tratti con caratteristiche differenti, per descriverne accuratamente la scabrezza lungo la sezione trasversale. I valori di scabrosità delle superfici utilizzati sono riportati al manuale del software, dov'è possibile trovare i coefficienti di Manning suddivisi per tipologia di alvei e di golene fluviali.
\\
...
\noindent Nel caso studio l'alveo presenta frequentemente condizioni di terreno sabbioso e ghiaioso ($ks=50$), delimitato da sponde e golene coperte da vegetazione ($ks=75$) e pareti artificiali perimetrali ($ks=15$).

\noindent La portata di progetto si è ricavata consultando il Piano di Bacino in cui si inserisce l'Ambito Regionale di Bacino 16 della Regione Liguria, contenente una sezione di analisi effettuate in corrispondenza delle zone interessate dallo Sturla. Di seguito vengono indicate le portate critiche indicate per tempi di ritorno di 50, 200, 500 anni:
\\
...
\section{Modellazione idraulica}
...
Nel caso di deflusso normale, le condizioni imposte sono di moto uniforme sia a monte che a valle. Per ottenere tali condizioni è necessario imporre la pendenza di moto uniforme alle estremità dell'alveo (comando \textit{Normal Depth}), in tal caso 0.02\% a monte e 2\% a valle.\\
Nel caso di regime con deflusso rigurgitato, invece, a valle si prefissa una quota di pelo libero da rispettare (comando \textit{Know WS}), da sommare all'altezza del fondo pari a 21.15 m.\\
...
\subsection{Profili di corrente}

\noindent Nel seguente paragrafo si riporta osservazioni e valutazioni condotte sui profili di corrente prodotti dalle simulazioni e sulla verifica idraulica del ponte.
...
\noindent In Figura 14, analizzando il corso d'acqua partendo da monte, è possibile notare le numerose variazioni di profondità che il profilo di corrente subisce lungo il suo percorso, dovute sia alle differenze di scabrosità che caratterizzano i diversi tratti del letto fluviale, che alla pendenza irregolare dell'alveo. \\

\begin{figure}[H]
\begin{minipage}[b]{8.5cm}
\noindent Le discontinuità nella pendenza del fondo sono tipiche di un corso d'acqua montano quale il torrente Sturla. Lungo tutto il percorso il fondo presenta dei tratti di forte ripidità, dove la corrente si riduce di profondità fino a intersecare il livello della critica. A valle, in particolare, la presenza di una rampa con forte pendenza genera un aumento localizzato della velocità di corrente, subito frenata con un risalto dissipativo al termine del dislivello.\\
...
\noindent Raddoppiare il coefficiente di Manning significa aumentare la scabrosità dell'alveo (a cui corrisponde un minor valore del coefficiente di Stricker), e questo produce un rallentamento della corrente e un generale incremento della profondità.
...
\noindent In Figura 15 e Figura 16, si nota come un aumento della portata immessa, corrispondente a eventi di crescente criticità, produce un generale incremento della profondità di corrente e una maggiore tendenza ad assumere un comportamento fluviale da parte del corso d'acqua
...
\noindent La verifica idraulica dev'essere condotta in ogni caso esamindando il tirante a monte del manufatto.
\textbf{A livello legislativo, il dimensionamento di un'opera d'attraversamento fluviale deve tener conto della quota di tirante raggiunta dalla corrente al verificarsi di eventi critici, con tempi di ritorno prestabiliti (quali?)}. In particolare, è necessario rispettare un franco di sicurezza dall'intradosso del ponte che permetta il libero sfogo della corrente in caso di evento eccezionale, senza che l'opera venga messa in pressione dal corso d'acqua.\\
...
\noindent Da una valutazione complessiva del contesto ambientale in cui si inserisce l'opera, è chiaro come le condizioni ambientali che caratterizzano il torrente siano favorevoli alle sviluppo di situazioni critiche per la tenuta degli argini e della struttura.\\
...
\begin{itemize}
    ...
    \item Collischonn W., “Hidrologia para engenharia e ciências ambientais”, 2013
    \item Ambito regionale di Bacino 16, Piano di Bacino - Stralcio sul rischio idrogeologico
\end{itemize}
\end{document}
